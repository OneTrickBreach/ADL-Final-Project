...
learned progression \gii{} $\to$ \gv{} is monotonic in both stochastic
and deterministic regimes, and the peak learned policy (\gv{}) beats the
...
\begin{enumerate}[nosep]
  ...
  \item Quantitative evidence that the learned progression is monotonic
    on both stochastic and deterministic evaluation: stochastic
    $1.80 \to 4.10 \to 7.40 \to 11.20$; deterministic
...
Under argmax, score also climbs monotonically
$-1.90 \to 0.60 \to 0.20 \to 2.90$, and \gv{}'s deterministic score
($2.90$) outperforms \giii{}'s stochastic score ($4.10$) by a factor
that matters: the policy does not need its sampling noise to work.
This is the opposite of the failure pattern where constrained policies
fire stochastically but freeze under argmax. The transition
\giv{} $\to$ \gv{} (stochastic $+3.80$, deterministic $+2.70$) is
particularly important: it shows the override turns a policy that was
